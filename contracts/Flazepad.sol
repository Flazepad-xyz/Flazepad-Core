...
contract FlazeCoinFactory is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function createFlazeCoinLaunch(
        ...
        if (useSocial && usePrivacy) {
            ...
            (bool ok1,) = devWallet.call{value: useFeatureFee + useFeatureFee}("");
            ...
            (bool ok2,) = devWallet.call{value: useFeatureFee}("");
            require(ok2, "ETH transfer failed");
        } else if (!useSocial && usePrivacy) {
            require(msg.value >= feeAmount + featuredAmounts[featureNumber] + useFeatureFee, "Insufficient Amount");
            (bool ok3,) = devWallet.call{value: useFeatureFee}("");
            require(ok3, "ETH transfer failed");
        } else if (!useSocial && !usePrivacy) {
            require(msg.value >= feeAmount + featuredAmounts[featureNumber], "Insufficient Amount");
        }
        (bool ok4,) = devWallet.call{value: feeAmount + featuredAmounts[featureNumber]}("");
        require(ok4, "ETH transfer failed");
        ...
        (bool ok5,) = payable(flazecoinLaunchAddr).call{value: additionalEthLp}("");
        require(ok5, "ETH transfer failed");
